...
tool_manager.add_tool(
    subtract_numbers, "subtract_numbers", "Subtract second number from first"
)
tool_manager.add_tool(
    multiply_numbers, "multiply_numbers", "Multiply two numbers together"
)
...
Always respond with the calculation result in a clear format like: 'The result is: {result}'.""",
    ...
    model="gpt-4-turbo",  # Use OpenAI model
    api_key=api_key,
)
...
agent_card = AgentCard(
    ...
    skills=[
        AgentSkill(
            ...
            examples=["What is 5 + 3?", "Calculate 10 - 7", "Multiply 4 and 6"],
        )
    ],
